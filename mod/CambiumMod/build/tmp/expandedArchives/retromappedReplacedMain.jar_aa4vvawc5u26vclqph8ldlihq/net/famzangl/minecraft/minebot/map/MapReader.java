...
public class MapReader implements ChunkListener {
	...
	final class WriteableImage {
		...
		public void renderAt(WorldData world, Chunk chunk, int dx, int dz) {
			int color = mode.getColor(world, chunk, chunk.field_76635_g * 16 + dx,
					chunk.field_76647_h * 16 + dz);
			getPaintingImage().setRGB(
					-pos.topLeftX + chunk.field_76635_g * 16 + dx,
					-pos.topLeftZ + chunk.field_76647_h * 16 + dz, color);
		}
	...
	protected static int getChunkHash(Chunk chunk) {
		ExtendedBlockStorage[] a = chunk.func_76587_i();
		if (a.length == 0) {
			return 0;
		}
	...
	final class MapReaderTask implements Runnable {
		...
		private void renderChunk(Chunk chunk) {
			...
			ImagePos pos = new ImagePos(chunk.field_76635_g * 16,
					chunk.field_76647_h * 16);

			MultiModeImage image = getImage(pos);
			int hash = getChunkHash(chunk);
			if (!image.isValidForChunkHash(chunk.field_76635_g, chunk.field_76647_h,
					hash)) {
				System.err.println("Abort rendering: Chunk hash has changed.");
				if (!wcm.chunkHashChanged(chunk.field_76635_g, chunk.field_76647_h)) {
					return;
				}
			}
			image.setChunkHash(chunk.field_76635_g, chunk.field_76647_h, hash);
	...
	public void tick(AIHelper helper) {
		...
		for (ChunkCoordIntPair d : chunkQueue.tickAndGet()) {
			Chunk chunkFromChunkCoords = helper.getMinecraft().field_71441_e
					.func_72964_e(d.field_77276_a, d.field_77275_b);
		...
		checkIsAlive(helper);
		EntityPlayerSP playerSP = helper.getMinecraft().field_71439_g;
		BlockPos newPlayer = playerSP == null ? null : helper
				.getPlayerPosition();
		int newLook = niceDegrees(playerSP == null ? 0
				: (int) playerSP.field_70177_z);
	...
	private void checkIsAlive(AIHelper helper) {
		...
		if (!isAlive && wasAlive && helper.getMinecraft().field_71439_g != null) {
	...
	private void loadAllChunks(AIHelper helper) {
		try {
			WorldClient theWorld = helper.getMinecraft().field_71441_e;
			if (theWorld == null) {
				return;
			}
			...
			for (Chunk chunk : list) {
				chunkQueue.offer(chunk.func_76632_l());
			}
	...
	public void addIcon(IconDefinition iconDefinition) {
		ImagePos pos = new ImagePos(iconDefinition.getPosition().func_177958_n(),
				iconDefinition.getPosition().func_177952_p());

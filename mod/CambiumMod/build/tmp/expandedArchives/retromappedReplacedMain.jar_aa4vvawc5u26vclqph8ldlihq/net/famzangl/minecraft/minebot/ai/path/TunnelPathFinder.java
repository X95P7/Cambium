...
public class TunnelPathFinder extends AlongTrackPathFinder {
	...
	protected void addTasksForTarget(BlockPos currentPos) {
		final int stepNumber = getStepNumber(currentPos.func_177958_n(),
		...
		BlockPos p1, p2;
		if (dx == 0) {
			p1 = currentPos.func_177982_a(addToSide, 0, 0);
			p2 = currentPos.func_177982_a(-addToSide, 1 + addToTop, 0);
		} else {
			p1 = currentPos.func_177982_a(0, 0, addToSide);
			p2 = currentPos.func_177982_a(0, 1 + addToTop, -addToSide);
		}
	...
	private void addBranchTask(BlockPos currentPos, int dx, int dz) {
		int branchMax = 0;
		for (int i = 1; i <= 4; i++) {
			if (!isSafeBranchPos(currentPos.func_177982_a(dx * i, 1, dz * i)))
				break;
			branchMax = i;
		}
		if (branchMax > 0) {
			addTask(new DestroyInRangeTask(currentPos.func_177982_a(dx * 1, 1, dz * 1),
					currentPos.func_177982_a(dx * branchMax, 1, dz * branchMax)));
		}
	}

	private boolean isSafeBranchPos(BlockPos pos) {
		return BlockSets.safeSideAndCeilingAround(world, pos)
				&& BlockSets.SAFE_SIDE.isAt(world, pos.func_177982_a(0, -1, 0));
	}
	...
	private void addTorchesTask(BlockPos currentPos, int dirX, int dirZ) {
		...
		positions.add(new BlockPos(currentPos.func_177958_n() + dirX * addToSide, currentPos
				.func_177956_o() + 1, currentPos.func_177952_p() + dirZ * addToSide));

		for (int i = addToSide; i >= 0; i--) {
			positions.add(new BlockPos(currentPos.func_177958_n() + dirX * i, currentPos
					.func_177956_o(), currentPos.func_177952_p() + dirZ * i));
		}

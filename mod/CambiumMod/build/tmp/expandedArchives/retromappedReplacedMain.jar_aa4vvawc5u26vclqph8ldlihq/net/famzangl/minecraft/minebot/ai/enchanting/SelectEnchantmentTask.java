...
public class SelectEnchantmentTask extends AITask {
	...
	public boolean isFinished(AIHelper h) {
		if (!(h.getMinecraft().field_71462_r instanceof GuiEnchantment)) {
			...
			return screen.field_147002_h.func_75139_a(0).func_75216_d()
					&& screen.field_147002_h.func_75139_a(0).func_75211_c()
							.func_77948_v();
		}
	}
	...
	private boolean attemptEnchanting(AIHelper h, ContainerEnchantment c,
			int slot) {
		if (c.field_75167_g[slot] == 0) {
			System.out.println("No enchantment levels computed yet.");
			return false;
		}
		if (h.getMinecraft().field_71439_g.field_71068_ca < c.field_75167_g[slot]) {
			System.out.println("Abort enchantment, not enough levels.");
			return slot > 0 ? attemptEnchanting(h, c, slot - 1) : false;
		}
		if (c.func_75140_a(h.getMinecraft().field_71439_g, slot)) {
			h.getMinecraft().field_71442_b.func_78756_a(c.field_75152_c,
					slot);

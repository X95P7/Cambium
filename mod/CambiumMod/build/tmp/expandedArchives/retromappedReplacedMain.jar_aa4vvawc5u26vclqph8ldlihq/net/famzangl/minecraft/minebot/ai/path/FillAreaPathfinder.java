...
public class FillAreaPathfinder extends MovePathFinder {
	...
	public FillAreaPathfinder(BlockCuboid cuboid,
		...
		currentFillLayer = cuboid.getMin().func_177956_o();
	...
	protected void noPathFound() {
		super.noPathFound();
		if (currentFillLayer < fillCuboid.getMax().func_177956_o()) {
			currentFillLayer++;
	...
	protected float rateDestination(int distance, int x, int y, int z) {
		...
		for (BlockPos s : AbstractBuildTask.STANDABLE) {
			int blockPosY = y - s.func_177956_o();
			if (y != currentFillLayer) {
				continue;
			}
			int blockPosX = x - s.func_177958_n();
			int blockPosZ = z - s.func_177952_p();
	...
	private boolean generatePlaceBlockTask(BlockPos currentPos) {
		for (BlockPos s : AbstractBuildTask.STANDABLE) {
			BlockPos placeAtPos = currentPos.func_177973_b(s);
			if (placeAtPos.func_177956_o() == currentFillLayer

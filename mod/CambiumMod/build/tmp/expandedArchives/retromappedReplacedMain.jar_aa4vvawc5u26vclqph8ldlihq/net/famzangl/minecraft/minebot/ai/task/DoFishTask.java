...
public class DoFishTask extends AITask {
	...
	public boolean isFinished(AIHelper h) {
		return revoked || h.getMinecraft().field_71439_g.field_71104_cf == null;
	}
	...
	private boolean fishIsCaptured(AIHelper helper) {
		...
		return minebotNetHandler
				.fishIsCaptured(helper.getMinecraft().field_71439_g.field_71104_cf);
	}
	...
	private boolean fishIsCapturedSP(AIHelper helper) {
		final EntityFishHook fishEntity = helper.getMinecraft().field_71439_g.field_71104_cf;
		if (fishEntity == null) {
			return false;
		}
		if (fishEntity.field_70181_x < -0.05) {
			LOGGER.trace(MARKER_FISH, "Fish motion: " + fishEntity.field_70181_x + ", " + fishEntity.field_70163_u);
			rightMotion--;
		} else {
			rightMotion = 2;

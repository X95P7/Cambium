...
public class OrebfuscatedMinePathFinder extends MineBySettingsPathFinder {
	...
	private static final BlockSet visibleMakingBlocks = new BlockSet(
			Blocks.field_150351_n, Blocks.field_150346_d).unionWith(targetBlocks).invert();
	...
	protected float rateDestination(int distance, int x, int y, int z) {
		if (y == preferedLayer && isGoodForOrebufscator(x, y, z)) {
			int d = ignoredAbs(x - searchCenter.func_177958_n(),
					preferedDirection.func_82601_c())
					+ ignoredAbs(z - searchCenter.func_177952_p(),
							preferedDirection.func_82599_e());

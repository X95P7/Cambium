...
public class MapDisplay extends JPanel implements FollowPlayerListener {
	...
	public void paint(Graphics g) {
		...
		int minPosX = ImagePos.round(mapCenter.func_177958_n() - getWidth()
				* blocksPerBasePixel / BASE_PIXEL / 2);
		int maxPosX = ImagePos.round(mapCenter.func_177958_n() + getWidth()
				* blocksPerBasePixel / BASE_PIXEL / 2);
		int minPosZ = ImagePos.round(mapCenter.func_177952_p() - getHeight()
				* blocksPerBasePixel / BASE_PIXEL / 2);
		int maxPosZ = ImagePos.round(mapCenter.func_177952_p() + getHeight()
				* blocksPerBasePixel / BASE_PIXEL / 2);
		...
		for (IconDefinition icon : icons) {
			...
			int x = blockToPanelX(icon.getPosition().func_177958_n()) - img.getWidth()
					/ 2;
			int y = blockToPanelY(icon.getPosition().func_177952_p()) - img.getHeight()
					/ 2;
	...
	private void drawMarks(Graphics g) {
		synchronized (markPositionMutex) {
			if (mark1 != null && mark2 != null) {
				int x1 = blockToPanelX(mark1.func_177958_n());
				int y1 = blockToPanelY(mark1.func_177952_p());
				int x2 = blockToPanelX(mark2.func_177958_n());
				int y2 = blockToPanelY(mark2.func_177952_p());
	...
	private void drawPlayer(Graphics g) {
		int x = blockToPanelX(player.playerPosition.func_177958_n());
		int y = blockToPanelY(player.playerPosition.func_177952_p());
	...
	private int blockToPanelX(int blockX) {
		return (blockX - mapCenter.func_177958_n()) * BASE_PIXEL / blocksPerBasePixel
				+ getWidth() / 2;
	}

	private int blockToPanelY(int blockZ) {
		return (blockZ - mapCenter.func_177952_p()) * BASE_PIXEL / blocksPerBasePixel
				+ getHeight() / 2;
	}

	private BlockPos getBlockPosition(Point panelPosition) {
		int blockX = (int) ((panelPosition.getX() - getWidth() / 2.0)
				* blocksPerBasePixel / BASE_PIXEL + mapCenter.func_177958_n());
		int blockZ = (int) ((panelPosition.getY() - getHeight() / 2.0)
				* blocksPerBasePixel / BASE_PIXEL + mapCenter.func_177952_p());
	...
	public String getToolTipText(MouseEvent event) {
		...
		return pos.func_177958_n() + "," + pos.func_177952_p();
	}
	...
	public void setMapPosition(BlockPos position, Point point) {
		...
		mapCenter = position.func_177963_a(-dx * getScale(), 0, -dy * getScale());

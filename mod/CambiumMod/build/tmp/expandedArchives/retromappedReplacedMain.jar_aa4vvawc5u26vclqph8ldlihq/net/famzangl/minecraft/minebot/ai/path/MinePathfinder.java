...
//	}
	...
	protected final boolean runSearch(BlockPos playerPosition) {
		...
		if (factors == null) {
			...
			for (ResourceLocation k : (Set<ResourceLocation>) Block.field_149771_c
					.func_148742_b()) {
				int id = Block.func_149682_b((Block) Block.field_149771_c
						.func_82594_a(k));
				// TODO: Do this better...
	...
	protected float rateDestination(int distance, int x, int y, int z) {
		...
		if (rating == Float.POSITIVE_INFINITY) {
			...
			if (preferedDirection != null
					&& preferedDirection.func_82601_c() != 0) {
				final int dx = x - current.func_177958_n();
				if (Math.signum(dx) != preferedDirection.func_82601_c()) {
					badDirectionMalus = dx * preferedDirectionInfluence;
				}
			} else if (preferedDirection != null
					&& preferedDirection.func_82599_e() != 0) {
				final int dz = z - current.func_177952_p();
				if (Math.signum(dz) != preferedDirection.func_82599_e()) {
	...
	protected void addTasksForTarget(BlockPos currentPos) {
		BlockPos top, bottom;
		int x = currentPos.func_177958_n();
		int y = currentPos.func_177956_o();
		int z = currentPos.func_177952_p();
		if (isOreBlock(x, y + 1, z)) {
			top = currentPos.func_177982_a(0, 1, 0);
		} else {
			top = currentPos;
		}
		if (isOreBlock(x, y, z)) {
			...
			bottom = currentPos.func_177982_a(0, 1, 0);
		}
		...
		for (int i = 2; i < 5; i++) {
			...
			if (isOreBlock(x, y + i, z)) {
				top = currentPos.func_177982_a(0, i, 0);
			} else {
				break;
			}
		}

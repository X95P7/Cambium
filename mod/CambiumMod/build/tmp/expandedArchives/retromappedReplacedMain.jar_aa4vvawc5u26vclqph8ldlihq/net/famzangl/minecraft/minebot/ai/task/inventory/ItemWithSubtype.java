...
public class ItemWithSubtype {
	...
	public ItemWithSubtype(ItemStack stack) {
		this(Item.func_150891_b(stack.func_77973_b()), stack.func_77952_i());
	}

	public ItemWithSubtype(int itemId, int itemDamage) {
		this.itemId = itemId;
		this.itemDamage = itemDamage;
		this.hasSubtype = getItem().func_77614_k();
	}
	...
	public Item getItem() {
		return Item.func_150899_d(itemId);
	}
	...
	public BlockWithDataOrDontcare getBlockType() {
		...
		if (item instanceof ItemBlock) {
			...
			if (hasSubtype) {
				int meta = getItem().func_77647_b(itemDamage);
				return new BlockWithData(itemBlock.field_150939_a, meta);
			} else {
				return new BlockWithDontcare(itemBlock.field_150939_a);
			}
		} else {
			return null;
		}
	...
	public static ItemWithSubtype fromTypeName(String name) {
		Item item = Item.func_111206_d(name);
		if (item == null) {
			return null;
		}
		return new ItemWithSubtype(Item.func_150891_b(item), 0);
	}
}

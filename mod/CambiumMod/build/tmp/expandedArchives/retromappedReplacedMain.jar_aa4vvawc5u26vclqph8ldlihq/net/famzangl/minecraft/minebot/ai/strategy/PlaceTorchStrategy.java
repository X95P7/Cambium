...
public class PlaceTorchStrategy extends AIStrategy {
	...
	public static class PosAndDir {
		...
		public BlockPos getPlaceOn() {
			return place.func_177972_a(dir);
		}
	...
	public boolean checkShouldTakeOver(AIHelper helper) {
		...
		EntityPlayerSP playerPosition = helper.getMinecraft().field_71439_g;
		BlockPos playerBlockPosition = helper.getPlayerPosition();
		if (!done.contains(playerBlockPosition)
				&& playerBlockPosition.func_177957_d(playerPosition.field_70165_t,
						playerBlockPosition.func_177956_o() + .5, playerPosition.field_70161_v) < .3 * .3) {
	...
	private void loadAttemptPositions(WorldData world) {
		...
		for (BlockPos p : new BlockPos[] { world.getPlayerPosition(),
				world.getPlayerPosition().func_177982_a(0, 1, 0) }) {
	...
	protected TickResult onGameTick(AIHelper helper) {
		...
		helper.faceSideOf(pos.getPlaceOn(), pos.dir.func_176734_d());
		if (inCurrentTick > 4
				&& helper
						.isFacingBlock(pos.getPlaceOn(), pos.dir.func_176734_d())) {

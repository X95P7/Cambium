...
public class TreePathFinder extends MovePathFinder {
	...
	private class LargeTreeState {
		...
		public LargeTreeState(BlockPos p) {
			this(p.func_177958_n(), p.func_177956_o(), p.func_177952_p());
		}
		...
		public void scanTreeHeight(WorldData world, BlockPos ignoredPlayerPos) {
			...
			for (int y = playerStartY; y < 255; y++) {
				...
				if (!BlockSets.LOGS.isAt(world, pos.func_177982_a(0, 3, 0)) || !allowedGroundBlocks.isAt(world, pos.func_177982_a(0, -1, 0))) {
					break;
				}
...
//				}
			...
			for (int y = playerStartY; y > 0; y--) {
				int trunkBlocks = countTrunkBlocks(world, y);
				BlockPos pos = getPosition(y);
				if (trunkBlocks < 1 || !allowedGroundBlocks.isAt(world, pos.func_177982_a(0, -1, 0))) {
					break;
				}
				minY = y;
			}
		...
		private int countTrunkBlocks(WorldData world, int topY) {
			...
			BlockCuboid trunk = new BlockCuboid(corner, corner.func_177982_a(1, 0, 1));
		...
		public void setYOffsetByPosition(BlockPos currentPos) {
			for (stairOffset = 0; stairOffset < 4; stairOffset++) {
				if (getPosition(currentPos.func_177956_o()).equals(currentPos)) {
					return;
				}
			}
		...
		public void addTasks(AIHelper h) {
			...
			if (!topReached) {
				for (int y = pos.func_177956_o() + 1; y < topY - TREE_TOP_OFFSET; y++) {
					BlockPos digTo = getPosition(y);
					if (!BlockSets.safeSideAndCeilingAround(world,
							digTo.func_177982_a(0, 1, 0))
							|| !BlockSets.safeSideAround(world, digTo)
							|| !BlockSets.SAFE_GROUND.isAt(world,
									digTo.func_177982_a(0, -1, 0))) {
						break;
					}
					addTask(new JumpMoveTask(digTo, lastPos.func_177958_n(),
							lastPos.func_177952_p()));
					lastPos = digTo;
				}
			}
			addTask(new TopReachedTask());
			// now destroy all
			for (int y = lastPos.func_177956_o(); y >= minY; y--) {
				BlockPos digTo = getPosition(y);
		...
		private void addReplantTasks(AIHelper h) {
			for (int i = 0; i < 4; i++) {
				BlockPos floorBase = getPosition(minY + i).func_177982_a(0, -i, 0);
				if (!BlockSets.SAFE_GROUND.isAt(world, floorBase.func_177982_a(0, -1, 0))
						|| !BlockSets.safeSideAround(world, floorBase)
						|| !BlockSets.safeSideAndCeilingAround(world,
								floorBase.func_177982_a(0, 1, 0))) {
					return;
				}
			}
		...
		private boolean isValidPlayerPosition(BlockPos pos) {
			return !(pos.func_177956_o() < playerStartY || pos.func_177956_o() >= topY || !getPosition(
					pos.func_177956_o()).equals(pos));
		}
	...
	protected void addTasksForTarget(BlockPos currentPos) {
		...
		for (int i = 2; i < TREE_HEIGHT; i++) {
			if (isLog(currentPos.func_177958_n(), currentPos.func_177956_o() + i,
		...
		for (int i = 2; i <= mineAbove; i++) {
			BlockPos pos = currentPos.func_177982_a(0, i, 0);
		...
		if (max > 0) {
			addTask(new DestroyInRangeTask(new BlockCuboid(currentPos.func_177982_a(0, 2,
					0), currentPos.func_177982_a(0, max, 0))));
		}
	...
	private boolean handleLargeTree(BlockPos currentPos) {
		for (BlockPos p : new BlockPos[] { currentPos,
				currentPos.func_177982_a(0, 0, -1), currentPos.func_177982_a(-1, 0, 0),
				currentPos.func_177982_a(-1, 0, -1), }) {
	...
	protected boolean runSearch(BlockPos playerPosition) {
		...
		if (BlockSets.AIR.isAt(world, playerPosition.func_177972_a(EnumFacing.DOWN))
				&& allowedGroundBlocks.isAt(world, playerPosition.func_177967_a(EnumFacing.DOWN, 2))) {
			// Bug: Desync during jump. TODO: Port this to all other pathfinders, find a common way to handle it safely.
			playerPosition = playerPosition.func_177972_a(EnumFacing.DOWN);
		}

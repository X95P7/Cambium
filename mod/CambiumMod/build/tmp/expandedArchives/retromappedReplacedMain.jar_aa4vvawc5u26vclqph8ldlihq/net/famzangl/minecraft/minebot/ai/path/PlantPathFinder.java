...
public class PlantPathFinder extends MovePathFinder {
	private static final BlockSet FARMLAND = new BlockSet(Blocks.field_150458_ak);
	private static final BlockSet NETHERWART_FARMLAND = new BlockSet(
			Blocks.field_150425_aM);

	private static final BlockSet FARMLANDABLE = new BlockSet(Blocks.field_150346_d,
	...
	private final class PlaceSeedsTask extends PlaceBlockAtFloorTask {
		...
		public boolean applyToDelta(WorldWithDelta world) {
			...
			Block b = Block.func_149634_a(anyPlaceItem);
			world.setBlock(pos, Block.func_149682_b(b), 0);
			return true;
		}
	}

	public enum PlantType {
		NORMAL(FARMLAND, Items.field_151014_N, Items.field_151172_bF, Items.field_151174_bG), WHEAT(
				FARMLAND, Items.field_151014_N), CARROT(FARMLAND, Items.field_151172_bF), POTATO(
				FARMLAND, Items.field_151174_bG), NETHERWART(NETHERWART_FARMLAND,
				Items.field_151075_bm);
	...
	private final class SeedFilter implements ItemFilter {
		...
		public boolean matches(ItemStack itemStack) {
			return itemStack != null && type.canPlantItem(itemStack.func_77973_b());
		}
	}
	...
	private static class UseHoeTask extends UseItemOnBlockAtTask {
		...
		public boolean applyToDelta(WorldWithDelta world) {
			if (FARMLANDABLE.isAt(world, getPos())) {
				world.setBlock(getPos(), Blocks.field_150458_ak);
			}
			return true;
		}
	}
	...
	private boolean isGrown(WorldData world, int x, int y, int z) {
		...
		if (Block.func_149729_e(blockWithMeta >> 4) instanceof BlockCrops) {
			final int metadata = blockWithMeta & 0xf;
			return metadata >= 7;
		} else if (new BlockMetaSet(Blocks.field_150388_bm, 3).isAt(world, x, y, z)) {
			return true;
		}
		return false;
	}
	...
	protected void addTasksForTarget(BlockPos currentPos) {
		if (BlockSets.AIR.isAt(world, currentPos)) {
			BlockPos farmlandPos = currentPos.func_177982_a(0, -1, 0);

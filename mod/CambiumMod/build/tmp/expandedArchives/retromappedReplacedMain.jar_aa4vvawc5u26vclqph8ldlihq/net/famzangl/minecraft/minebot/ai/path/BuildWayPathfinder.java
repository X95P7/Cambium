...
public class BuildWayPathfinder extends AlongTrackPathFinder {
	...
	public BuildWayPathfinder(EnumFacing dir, BlockPos pos) {
		this(dir.func_82601_c(), dir.func_82599_e(), pos.func_177958_n(), pos
				.func_177956_o() + 1, pos.func_177952_p());
	}
	...
	private abstract class WayPiece {
		...
		public void addConstructionTasks(BlockPos currentPos) {
			...
			if (placeTorch) {
				addTask(new GetOnHotBarTask(new BlockItemFilter(Blocks.field_150478_aa)));
			}
			...
			for (int i = 1; i < width; i++) {
				...
				addTask(new SlabBuildTask(current, FLOOR.getBlock())
						.getPlaceBlockTask(getPos(i - 1, 0).func_177973_b(current)));
			}
		...
		protected final void addNearSide() {
			...
			addTask(task.getPlaceBlockTask(getPos(1, 1).func_177973_b(getPos(0, 0))));
		}
		...
		protected final void addFarSide() {
			final AbstractBuildTask task = new BlockBuildTask(getPos(width, -1),
					new BlockWithDontcare(BRIDGE_SIDE));
			addTask(task
					.getPlaceBlockTask(getPos(-1, 1).func_177973_b(getPos(0, 0))));
		}
	}
		...
		protected void addFarSideBuildTasks() {
			...
			if (placeTorch) {
				addTask(new PlaceTorchSomewhereTask(Arrays.asList(
						...
						getPos(width, 1)), getForwardDirection().func_176735_f(),
						EnumFacing.DOWN));
			}
		}
	}
	...
	protected void addTasksForTarget(BlockPos currentPos) {
		final int currentStep = getStepNumber(currentPos.func_177958_n(),
				currentPos.func_177952_p());
		if (currentPos.func_177956_o() == cy - 1
	...
	public boolean addContinuingTask(BlockPos playerPosition) {
		System.out.println("Seatch at " + playerPosition + ", on track: "
				+ isOnTrack(playerPosition.func_177958_n(), playerPosition.func_177952_p())
				+ ", cy= " + (cy - 1) + " right block: "
				+ new BlockSet(FLOOR.slabBlock).isAt(world, playerPosition));
		if (isOnTrack(playerPosition.func_177958_n(), playerPosition.func_177952_p())
				&& playerPosition.func_177956_o() == cy - 1
				&& new BlockSet(FLOOR.slabBlock).isAt(world, playerPosition)) {
			final int currentStep = getStepNumber(playerPosition.func_177958_n(),
					playerPosition.func_177952_p());
			final WayPiece next = getSuggestedWayType(currentStep + 1);
			if (!next.isDone()) {
				next.addConstructionTasksFromPrevoius(playerPosition.func_177982_a(0, 1,
						0));
				return true;
			}
		}

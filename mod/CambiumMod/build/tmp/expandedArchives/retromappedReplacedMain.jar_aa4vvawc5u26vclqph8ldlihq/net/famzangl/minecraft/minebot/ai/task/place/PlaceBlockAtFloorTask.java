...
public class PlaceBlockAtFloorTask extends AITask {
	...
	protected boolean isFacing(AIHelper h, EnumFacing dir) {
		BlockPos facingBlock = getPlaceAtPos().func_177972_a(dir);
		return h.isFacingBlock(facingBlock, dir.func_176734_d(), getSide(dir));
	}
	...
	protected final BlockPos getPlaceAtPos() {
		return pos.func_177982_a(0, getRelativePlaceAtY(), 0);
	}
	...
	protected void faceBlock(AIHelper h, TaskOperations o) {
		h.faceSideOf(getPlaceAtPos().func_177972_a(EnumFacing.DOWN), EnumFacing.UP);
	}
	...
	protected boolean isAtDesiredHeight(AIHelper h) {
		return h.getMinecraft().field_71439_g.func_174813_aQ().field_72338_b >= getPlaceAtPos()
				.func_177956_o();
	}

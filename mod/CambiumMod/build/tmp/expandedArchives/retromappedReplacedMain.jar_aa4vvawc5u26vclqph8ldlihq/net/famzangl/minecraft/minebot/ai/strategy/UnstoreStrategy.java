			...
			for (int inventorySlot = 0; inventorySlot < 36; inventorySlot++) {
				...
				if (mainInventory[inventorySlot] != null) {
					if (!new SameItemFilter(mainInventory[inventorySlot])
						...
						continue;
					} else if (mainInventory[inventorySlot].field_77994_a >= Math
							.min(mainInventory[inventorySlot].func_77976_d(),
									slot.amount)) {
						System.out
				...
				protected int getFromStack(AIHelper h) {
					if (fromStack == -2) {
						...
						SameItemFilter filter = new SameItemFilter(
								slot.getFakeMcStack());
						List<Slot> inventorySlots = screen.field_147002_h.field_75151_b;
						int fromStackRating = -1;
						int missing = getMissingAmount(h,
								getSlotContentCount(screen.field_147002_h
										.func_75139_a(getToStack(h))));
						for (int i = 0; i < inventorySlots.size() - 36; i++) {
							Slot s = inventorySlots.get(i);
							if (filter.matches(s.func_75211_c())) {
								int rating = rateSize(h,
										s.func_75211_c().field_77994_a, missing);
	...
	private static class UnstorePathFinder extends BlockRangeFinder {
		...
		protected void addTasksForTarget(BlockPos currentPos) {
			...
			for (final ChestData c : chests) {
				ItemStack[] inventory = helper.getMinecraft().field_71439_g.field_71071_by.field_70462_a;

...
public class ClearAreaPathfinder extends MovePathFinder {
	...
	private static final BlockSet CLEARED_BLOCKS = new BlockSet(Blocks.field_150350_a,
	...
	public ClearAreaPathfinder(BlockCuboid area,
		...
		this.area = area;
		this.mode = mode;
		topY = area.getMax().func_177956_o();
		if (block == null) {
	...
	protected float rateDestination(int distance, int x, int y, int z) {
		if (isInArea(x, y, z)
				&& (!isTemporaryCleared(x, y, z) || !isTemporaryCleared(x,
						y + 1, z) && y < area.getMax().func_177956_o())) {
			final float bonus = 0.0001f * (x - area.getMin().func_177958_n()) + 0.001f
					* (y - area.getMin().func_177956_o());
			int layerMalus;
			if (topY <= y) {
				layerMalus = 5;
			...
			return distance + bonus + layerMalus + (area.getMax().func_177956_o() - y)
					* 2;
		} else {
			return -1;
		}
	...
	protected void addTasksForTarget(BlockPos currentPos) {
		...
		for (int i = 1; i < mode.maxHeight; i++) {
			final BlockPos pos = currentPos.func_177982_a(0, i, 0);
			if (pos.func_177956_o() <= area.getMax().func_177956_o()) {
				top = pos;
			}
		}
		// avoid gravel/...
		while (!BlockSets.SAFE_CEILING.isAt(world, top)
				&& top.func_177956_o() > currentPos.func_177956_o() + 1) {
			top = top.func_177982_a(0, -1, 0);
		}
	...
	public int getToClearCount(AIHelper helper) {
		...
		AreaTopVisitor v = new AreaTopVisitor(area.getMin().func_177956_o());
		area.accept(v, world);
		topY = v.newTopY;

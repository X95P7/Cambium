...
public class MineNerbyPathFinder extends MineBySettingsPathFinder {
	...
	protected void onPreRunSearch(BlockPos playerPosition) {
		playerX = playerPosition.func_177958_n();
		playerY = playerPosition.func_177956_o();
		playerZ = playerPosition.func_177952_p();
		possiblePositions.clear();
		alreadyScanned.clear();

		findPossiblePositionsAround(playerPosition.func_177982_a(0, 1, 0));
	...
	private void findPossiblePositionsAround(BlockPos add) {
		if (!isNearbyArea(add.func_177958_n(), add.func_177956_o(), add.func_177952_p())
				|| alreadyScanned.get(getIndex(add))) {
			return;
		}
		alreadyScanned.set(getIndex(add));

		if (super.isOreBlock(add.func_177958_n(), add.func_177956_o(), add.func_177952_p())) {
		...
		if (BlockSets.FEET_CAN_WALK_THROUGH.isAt(world.getCurrentState(), add)) {
			for (EnumFacing d : EnumFacing.values()) {
				findPossiblePositionsAround(add.func_177972_a(d));
			}
		}
	}
	...
	private boolean isNearbyArea(int cx, int cy, int cz) {
		return Math.abs(playerX - cx) <= RADIUS_HORIZONTAL
				...
				&& (activationPosition == null || (Math.abs(activationPosition
						.func_177958_n() - cx) <= ACTIVATE_RADIUS_HORIZONTAL
						&& Math.abs(activationPosition.func_177956_o() - cy) <= ACTIVATE_RADIUS_VERTICAL && Math
						.abs(activationPosition.func_177952_p() - cz) <= ACTIVATE_RADIUS_HORIZONTAL
				));
	}

	private final int getIndex(BlockPos p) {
		return getIndex(p.func_177958_n(), p.func_177956_o(), p.func_177952_p());
	}

...
public class CommandMove {
		...
		protected TickResult onGameTick(AIHelper helper) {
			if (!moved) {
				...
				while (helper.buildManager.peekNextTask() != null) {
					...
					BlockPos nextPos = next.getForPosition().func_177971_a(relative);
					if (rotate != null) {
						nextPos = rotate.apply(nextPos.func_177973_b(center)).func_177971_a(
								center);
					}
					if (mirror == MirrorDirection.EAST_WEST) {
						// mirror on x axes
						nextPos = new BlockPos(center.func_177958_n() * 2
								- nextPos.func_177958_n(), nextPos.func_177956_o(),
								nextPos.func_177952_p());
					} else if (mirror == MirrorDirection.NORTH_SOUTH) {
						nextPos = new BlockPos(nextPos.func_177958_n(), nextPos.func_177956_o(),
								center.func_177952_p() * 2 - nextPos.func_177952_p());
					}
	...
	public enum Rotate {
		...
		public BlockPos apply(BlockPos blockPos) {
			switch (this) {
			case CLOCKWISE:
				return new BlockPos(-blockPos.func_177952_p(), blockPos.func_177956_o(),
						blockPos.func_177958_n());
			case COUNTERCLOCKWISE:
				return new BlockPos(blockPos.func_177952_p(), blockPos.func_177956_o(),
						-blockPos.func_177958_n());
			case HALF:
				return new BlockPos(-blockPos.func_177958_n(), blockPos.func_177956_o(),
	...
	public static AIStrategy run(
		...
		return new MoveStrategy(Pos.ZERO.func_177967_a(dir, howMuch), null, null);
	}

...
public class MoveScanner {
	...
	public static class ScannedChunk implements Comparable<ScannedChunk> {
		...
		public void scanBlocks(Chunk chunk, int chunkY, ScannerPolicy policy) {
			ExtendedBlockStorage storage = chunk.func_76587_i()[chunkY];
			char[] array = storage.func_177487_g();
			for (int y = 0; y < 16; y++) {
				for (int z = 0; z < 16; z++) {
			...
			ClassInheritanceMultiMap entities = chunk.func_177429_s()[chunkY];
			for (Object o : entities) {
				Entity e = (Entity) o;
				int distance = policy.getDangerDistanceFor(e);
				if (distance > 0) {
					int lx = ((int) Math.floor(e.field_70165_t + .5)) & 0xf;
					int ly = ((int) Math.floor(e.field_70165_t + .5)) & 0xf;
					int lz = ((int) Math.floor(e.field_70165_t + .5)) & 0xf;
	...
	public ScannedChunk getChunk(int chunkX, int chunkY, int chunkZ) {
		...
		synchronized (chunks) {
			...
			if (chunks[i] == null) {
				...
				Chunk mcChunk = helper.getMinecraft().field_71441_e
						.func_72964_e(chunkX, chunkZ);
				scheduler

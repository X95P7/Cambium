...
public class AIController extends AIHelper implements IAIControllable {
    ...
    public void onPlayerTick(ClientTickEvent evt) {
        ...
        if (getMinecraft().func_147113_T()) {
            getMinecraft().field_71474_y.field_82881_y = false;
        }
        ...
        if (evt.phase != ClientTickEvent.Phase.START) {
            return;
        } else if (getMinecraft().field_71439_g == null || getMinecraft().field_71441_e == null) {
        ...
        LOGGER.debug(MARKER_STRATEGY, "Strategy game tick. World time: " + getMinecraft().field_71441_e.func_82737_E());
        testUngrabMode();
    ...
    private boolean isStopPressed() {
        return stop.func_151468_f() || stop.func_151470_d() || Keyboard.isKeyDown(stop.func_151463_i());
    }
    ...
    public void drawHUD(RenderTickEvent event) {
        ...
        int y = 10;
        for (String s : str) {
            getMinecraft().field_71466_p.func_175063_a(
                s,
                res.func_78326_a() - getMinecraft().field_71466_p.func_78256_a(s) - 10, 
                y, 16777215);
            y += 15;
        }
    }
    ...
    private synchronized void startUngrabMode() {
        ...
        getMinecraft().field_71417_B.func_74373_b();
        getMinecraft().field_71415_G = true;
        if (!(getMinecraft().field_71417_B instanceof UngrabMouseHelper)) {
            LOGGER.trace(MARKER_MOUSE, "Storing old mouse helper.");
            oldMouseHelper = getMinecraft().field_71417_B;
        }
        getMinecraft().field_71417_B = new UngrabMouseHelper();
    ...
    private synchronized void testUngrabMode() {
        ...
        if (oldMouseHelper != null) {
            if ((userTookOver() || !displayWasActiveSinceUngrab) && Display.isActive()) {
                ...
                getMinecraft().field_71417_B = oldMouseHelper;
                getMinecraft().field_71415_G = false;
                getMinecraft().func_71381_h();
                oldMouseHelper = null;
            }
        }
    }
    ...
    public void drawMakers() {
        final Entity view = getMinecraft().func_175606_aa();

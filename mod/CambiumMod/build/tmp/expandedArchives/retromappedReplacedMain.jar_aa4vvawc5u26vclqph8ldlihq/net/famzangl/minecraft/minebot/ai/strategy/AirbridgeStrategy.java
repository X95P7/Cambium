...
public class AirbridgeStrategy extends TaskStrategy {
	...
	public static SneakAndPlaceAtHalfTask getBuildHafslabTask(
		...
		return new SneakAndPlaceAtHalfTask(walkToPos, SLABS_FILTER,
				beforeBuild, buildPos, buildPos.func_177956_o() + .5,
	...
	private static final class DynamicWaitTask extends AITask {
		...
		@Override
		public boolean isFinished(AIHelper h) {
			return h.getMinecraft().field_71441_e.func_82737_E() >= startTime + LAG_TEST_DELAY;
		}
		...
		public AITask getTrigger() {
			return new RunOnceTask() {
				...
				protected void runOnce(AIHelper h, TaskOperations o) {
					startTime = h.getMinecraft().field_71441_e.func_82737_E();
				}
			};
		}
	}
	private class BuildHalfslabVisitor implements AreaVisitor {
		...
		public void visit(WorldData world, int x, int y, int z) {
			...
			if (buildPos.func_177982_a(0, -1, 0).equals(pos)) {
	...
	private BlockCuboid getSidewardsArea(BlockPos center) {
		return new BlockCuboid(center, center).extend(toLeft,
				direction.func_176746_e()).extend(toRight, direction.func_176735_f());
	}
	...
	protected void searchTasks(AIHelper helper) {
		...
		for (int i = 0; i < 9 * 4; ++i) {
			if (SLABS_FILTER.matches(helper.getMinecraft().field_71439_g.field_71071_by
					.func_70301_a(i))) {
				slabsFound = true;
			}
		}
		...
		while (isHalfslabAt(world, buildPos) && isInLength(buildPos)
			...
			buildPos = buildPos.func_177971_a(direction.func_176730_m());
			steps++;
		}
		...
		if (beforeBuild == null) {
			BlockPos before = buildPos.func_177973_b(direction.func_176730_m());
			if (isAirAt(world, buildPos) && isHalfslabAt(world, before)) {
				...
				addTask(new WalkTowardsTask(before.func_177958_n(), before.func_177952_p(), null));
				...
				LOGGER.info(MARKER_PROGRESS,
						"No valid half slab found. Required area is: "
								+ getSidewardsArea(buildPos.func_177982_a(0, -1, 0)));
				}
			}
			return;
		}
		...
		if (isAirAtAndBelow(world, buildPos)) {
			...
			BlockCuboid area = getSidewardsArea(buildPos.func_177982_a(0, -1, 0));
			BlockArea toPlace = new BlockFilteredArea(area, BlockSets.AIR);
			if ((buildPos.func_177958_n() & 1) == (buildPos.func_177952_p() & 1)) {
			...
			// walk
			addTask(new WalkTowardsTask(buildPos.func_177958_n(), buildPos.func_177952_p(), null));
	...
	private boolean isInLength(BlockPos pos) {
		return length < 0 || start.func_177951_i(pos) + .5 < length * length;		
	}
	...
	private boolean isAirAtAndBelow(WorldData world, BlockPos buildPos) {
		...
		BlockCuboid floor = getSidewardsArea(buildPos.func_177982_a(0, -1, 0));
	...
	private boolean isAirAt(WorldData world, BlockPos buildPos) {
		return (buildPos.func_177956_o() > 255 || BlockSets.AIR.isAt(world,
				getSidewardsArea(buildPos)))
				&& (buildPos.func_177956_o() > 254 || BlockSets.AIR.isAt(world,
						getSidewardsArea(buildPos.func_177982_a(0, 1, 0))));
	}

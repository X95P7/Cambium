...
public class LayRailPathFinder extends AlongTrackPathFinder {
	...
	protected float rateDestination(int distance, int x, int y, int z) {
		if (isRedstoneBlockPosition(x, y, z)
				&& !new BlockSet(Blocks.field_150451_bX).isAt(world, x, y, z)) {
			return distance + 2;
	...
	protected void addTasksForTarget(BlockPos currentPos) {
		if (isRedstoneBlockPosition(currentPos.func_177958_n(), currentPos.func_177956_o(),
				currentPos.func_177952_p())) {
			// For those server lags
			addTask(new UpwardsMoveTask(currentPos.func_177982_a(0, 1, 0),
					new BlockItemFilter(Blocks.field_150451_bX)));
		} else if (placeAccRail(currentPos.func_177958_n(), currentPos.func_177952_p())) {
			if (!new BlockSet(Blocks.field_150451_bX).isAt(world, currentPos.func_177982_a(0, -1, 0))
					&& BlockSets.safeSideAround(world, currentPos.func_177982_a(0,-1,0))
					&& BlockSets.SAFE_GROUND.isAt(world, currentPos.func_177982_a(0,-2,0))) {
				addTask(new DownwardsMoveTask(currentPos.func_177982_a(0, -1, 0)));
				addTask(new UpwardsMoveTask(currentPos, new BlockItemFilter(
						Blocks.field_150451_bX)));
			}
			placeRail(currentPos, Blocks.field_150318_D);
		} else {
			placeRail(currentPos, Blocks.field_150448_aq);
		}
	}

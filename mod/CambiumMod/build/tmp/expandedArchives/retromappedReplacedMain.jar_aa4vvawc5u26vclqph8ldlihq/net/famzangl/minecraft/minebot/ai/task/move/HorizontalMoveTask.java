...
public class HorizontalMoveTask extends AITask implements CanPrefaceAndDestroy {
	private static final BlockSet hardBlocks = new BlockSet(Blocks.field_150343_Z);
	...
	public void runTick(AIHelper h, TaskOperations o) {
		if (needDestroyHead(h.getWorld())) {
			BlockPos upper = pos.func_177982_a(0, 1, 0);
			if (hardBlocks.isAt(h.getWorld(), upper)) {
				hasObsidianUpper = true;
			}
			h.faceAndDestroyWithHangingBlock(pos.func_177982_a(0, 1, 0));
		} else {
			if (needDestroyFoot(h.getWorld())) {
				...
				h.walkTowards(pos.func_177958_n() + 0.5, pos.func_177952_p() + 0.5, doJump(h),
						!nextIsFacing);
			}
		}
	}
...
//	}
	...
	public List<BlockPos> getPredestroyPositions(AIHelper helper) {
		...
		if (needDestroyHead(world)) {
			arrayList.add(pos.func_177982_a(0, 1, 0));
		}
	...
	private boolean needDestroyHead(WorldData world) {
		return !BlockSets.HEAD_CAN_WALK_TRHOUGH.isAt(world, pos.func_177982_a(0, 1, 0));
	}

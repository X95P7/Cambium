...
public class BlockSet implements Iterable<BlockWithData> {
	...
	public BlockSet(Block... blocks) {
		this();
		for (Block b : blocks) {
			setBlock(Block.func_149682_b(b));
		}
	}
	...
	public boolean contains(Block block) {
		return contains(Block.func_149682_b(block));
	}

	@Deprecated
	public boolean contains(IBlockState meta) {
		return contains(meta.func_177230_c());
	}
	...
	public List<BlockPos> findBlocks(WorldData world, BlockPos around,
		...
		for (int x = around.func_177958_n() - radius; x <= around.func_177958_n() + radius; x++) {
			for (int z = around.func_177952_p() - radius; z <= around.func_177952_p() + radius; z++) {
				for (int y = around.func_177956_o() - radius; y <= around.func_177956_o()
						+ radius; y++) {
					if (isAt(world, x, y, z)) {
	...
	protected String getForBlock(int blockId) {
		if (contains(blockId)) {
			return Block.func_149729_e(blockId).func_149732_F() + " ("
					+ blockId + ")";
		} else {
			return null;
		}
